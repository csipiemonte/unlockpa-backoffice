...
function CurrentProjectID() {
	...
	return "{1B294467-F675-48C8-9632-26D78A5119EB}";
}
...
class AdvancedSecurity
{
	...
	public function validateUser(&$usr, &$pwd, $autologin, $provider = "")
	{
		...
		if (!$valid) {
			...
			# ### CSI DRUPAL #### begin
			include_once('csidrupal/phpasshashedpassword.php');
			$HP = new PhpassHashedPassword(16);
			if(!$valid) $valid = $HP->check($pwd, $rs->fields('pass'));
			# ### CSI DRUPAL #### end
					...
					if ($valid) {
						...
						$this->setCurrentUserName($rs->fields('mail')); // Load user name
...
function MenuItem_Adding($item) {
   ...
   // ELIMINO DAL MENU LA VOCE "Lookup Tables" PER GLI UTENTI GENERICI
   // echo $item->Text;

   $var_name = strval ($item->Text);
  if (CurrentUserLevel() == 0)
  {
	 if ($var_name == "Lookup Tables")
		{

  	     // echo "<br>Eliminato menu per utenti non admin :".$var_name;
		 return FALSE;
		}
  }
  else
  {
	 if ($var_name == "Lookup Tables")
		...
		// echo "<br>Menu NON ELIMINATO per utenti admin :".$var_name;
		}
  }
	return TRUE;
}
