...
// Parametri relativi all'utente loggato
if (false)
{
  echo "CurrentUserName=".CurrentUserName()." -- ";
  echo "CurrentUserID=".CurrentUserID()." -- ";
  echo "CurrentUserLevel=".CurrentUserLevel()." -- ";
  echo "CurrentUserInfo(name)=".CurrentUserInfo("name")." -- ";
  echo "CurrentUserInfo(mail)=".CurrentUserInfo("mail")." -- ";
  echo "CurrentUserInfo(langcode)=".CurrentUserInfo("langcode")." -- ";
  echo "CurrentUserInfo(fk_comune)=".CurrentUserInfo("fk_comune")." -- ";
  echo "VEDI SERVER EVENTS AND CLIENT SCRIPTS  nell'help della versione utilizzata (i comandi cambiano tra una versione e un'altra)";
}

  //echo "Token_session=".$_SESSION["token_session"];
  //echo "Credenziali=".$_SESSION["credenziali"];
//  echo $_SESSION["Utente"];
 //  echo $_SESSION["Password"];
// echo base64_encode($_SESSION["Utente"].":".$_SESSION["Password"]);
...
setTimeout(() => {
inputVal = $('#frisposteedit #r_id_domanda #el_risposte_id_domanda .form-control-plaintext').val(),
$('#frisposteedit #r_id_domanda #el_risposte_id_domanda').prepend(inputVal),
$('#frisposteedit #r_id_domanda #el_risposte_id_domanda input').css('display', 'none'),
tableRisposteLista = $('#tbl_rispostelist tr').length
for(i=0; i< tableRisposteLista; i++) {
inputValRisposte = $('#el'+ i +'_risposte_domanda .form-control-plaintext').val(),
$('#el'+ i +'_risposte_domanda span').prepend(inputValRisposte),
$('#el'+ i +'_risposte_domanda input').css('display', 'none')
}
}, 200);
</script>
<?php } ?>
<link rel="shortcut icon" type="image/x-icon" href="<?php echo $RELATIVE_PATH ?>unlockpa.ico"><link rel="icon" type="image/x-icon" href="<?php echo $RELATIVE_PATH ?>unlockpa.ico">
...
<div class="wrapper ew-layout">
	...
	<aside class="<?php echo $SIDEBAR_CLASS ?>">
		...
		<a href="#" class="brand-link">
			<span class="brand-text">unlockPA</span>
		</a>
		<!-- Sidebar -->
		<div class="sidebar">
			<!-- Sidebar Menu -->
