...
class audittrail extends DbTable
{
	...
	public function __construct()
	{
		...
		// Update Table
		$this->UpdateTable = "\"unlockpa\".\"audittrail\"";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('audittrail', 'audittrail', 'x_id', 'id', '"id"', 'CAST("id" AS varchar(255))', 3, -1, FALSE, '"id"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "\"unlockpa\".\"audittrail\"";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->id->setDbValue($conn->getOne("SELECT currval('audittrail_id_seq'::regclass)"));
